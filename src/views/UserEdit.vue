    ...
    <div>
      <div>
        <div>
          ...
          <el-table :data="userProAndTask" stripe border style="width: 100%">
            <el-table-column prop="projectName" label="项目名" width="300">
            </el-table-column>
            <el-table-column prop="taskId" label="任务id" width="180">
            </el-table-column>
            <el-table-column prop="taskDetail" label="任务细节描述" width="300">
            </el-table-column>
            <el-table-column label="编辑">
              <template slot-scope="scope">
                <el-button size="mini" type="danger" width="100"
                  @click="handleDelete(scope.$index, scope.row, scope.row.taskId)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
          ...
          <el-form :model="form">
            ...
            <el-form-item>
              <el-select v-model="optionProValue" placeholder="请选择项目" @change="currentProSel">
                <el-option v-for="item in allPros" :key="item.projectId" :label="item.projectName" :value="item.projectId">
                </el-option>
              </el-select>
            </el-form-item>
            <!-- 选择任务 -->
            <el-form-item>
              <el-select v-model="optionTaskValue" placeholder="请选择任务id" @change="taskIsChange = true">
                <el-option v-for="item in allTasks" :key="item" :label="item" :value="item">
                </el-option>
              </el-select>
            </el-form-item>
            <!-- 选择流程 -->
            <el-form-item>
              <el-select v-model="optionProcessValue" placeholder="请选择流程" @change="processIsChange = true">
                <el-option v-for="item in processes" :key="item.value" :label="item.lable" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
  ...
  export default {
    data() {
      return {
        ...
        allTasks: [181,182,183,184,185,186,187,188,189,190,191,192,193,194,195,196,197,198,199,200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219],
        userProAndTask: [],
    ...
    methods: {
      ...
      handleDelete(index, row, taskid) {
        console.log("deleTaskrow:",row);
        this.userProAndTask.splice(index, 1);
        
        // 删除任务
        var uid = Cookie.get("editUser")
        var tid = taskid
        delUserTask(uid, tid, 1).then( res => {
          console.log("delUserTask:",res.data.message)
        }
      ...
      handleAddTask() {
        ...
        if (this.proIsChange === true && this.taskIsChange === true && this.processIsChange === true) {
          ...
          // 添加 de 代码
          console.log("newTask",this.userProAndTask)
          this.newtask.taskId = this.optionTaskValue
          this.userProAndTask.push(this.newtask)
          console.log("newTask:",this.userProAndTask)
